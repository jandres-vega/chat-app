"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createChat = void 0;
const mongoose_1 = require("mongoose");
const services_1 = require("../../../services");
const serviceChat = new services_1.ChatService();
const createChat = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { participant_one, participant_two } = req.body;
        const idOne = new mongoose_1.Types.ObjectId(participant_one);
        const idTwo = new mongoose_1.Types.ObjectId(participant_two);
        const response = yield serviceChat.createChat(idOne, idTwo);
        res.status(200).send(response);
    }
    catch (e) {
        next(e);
    }
});
exports.createChat = createChat;
